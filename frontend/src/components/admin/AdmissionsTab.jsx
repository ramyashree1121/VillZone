...
import { Eye, Search, Download, CheckCircle, XCircle, Save, CheckSquare } from 'lucide-react';
...
export default function AdmissionsTab({ token, showSuccess }) {
  ...
  const handleStatusUpdate = async (status, skipConfirm = false) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/admissions/${selectedApp._id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'Approved': return <span className="bg-green-100 text-green-700 px-2 py-1 rounded text-[10px] font-black uppercase">Approved</span>;
  ...
  const statusOptions = ['New', 'Under Review', 'Contacted', 'Documents Pending', 'Approved', 'Confirmed', 'Rejected'];
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
        ...
        <div className="bg-white border rounded-xl p-4 shadow-sm text-center">
          ...
          <p className="text-2xl font-black text-orange-500">{admissions.filter(a => a.status === 'Under Review' || a.status === 'Contacted' || a.status === 'Documents Pending').length}</p>
        </div>
        <div className="bg-white border rounded-xl p-4 shadow-sm text-center">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Approved</p>
          <p className="text-2xl font-black text-green-500">{admissions.filter(a => a.status === 'Approved').length}</p>
        </div>
        <div className="bg-white border rounded-xl p-4 shadow-sm text-center">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Confirmed</p>
          <p className="text-2xl font-black text-emerald-600">{admissions.filter(a => a.status === 'Confirmed').length}</p>
        </div>
        <div className="bg-white border rounded-xl p-4 shadow-sm text-center">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#E2E8F0] overflow-hidden">
        <div className="p-4 border-b border-[#E2E8F0] flex flex-wrap gap-4 justify-between bg-slate-50">
            ...
            <div className="w-full md:w-2/5 flex flex-col bg-white">
              ...
              <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50">
                ...
                <div className="grid grid-cols-2 gap-3 mb-8">
                  <button onClick={() => handleStatusUpdate(editStatus, true)} className="col-span-2 flex justify-center items-center gap-2 bg-slate-800 text-white p-3 rounded-xl font-bold text-sm hover:bg-slate-700 transition-colors">
                    <Save size={16} /> Save Status
                  </button>
                  <button onClick={() => handleStatusUpdate('Approved')} className="flex justify-center items-center gap-2 bg-green-500 text-white p-3 rounded-xl font-bold text-sm hover:bg-green-600 transition-colors">
                    <CheckCircle size={16} /> Approve
                  </button>
                  <button onClick={() => handleStatusUpdate('Confirmed')} className="flex justify-center items-center gap-2 bg-emerald-600 text-white p-3 rounded-xl font-bold text-sm hover:bg-emerald-700 transition-colors">
                    <CheckSquare size={16} /> Confirm
                  </button>
                  <button onClick={() => handleStatusUpdate('Rejected')} className="col-span-2 flex justify-center items-center gap-2 bg-rose-500 text-white p-3 rounded-xl font-bold text-sm hover:bg-rose-600 transition-colors">
                    <XCircle size={16} /> Reject Admission
                  </button>
                </div>
