...
const CampusVisitsTab = ({ token }) => {
  ...
  const fetchVisits = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/campus-visits', {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/campus-visits/${id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/campus-visits/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
                  ...
                  <td className="p-3">
                    {editingId === v._id ? (
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${
                        v.status === 'New Lead' ? 'bg-blue-100 text-blue-800' :
